...
namespace Carnivale
{
    public class JobDriver_HaulToCarriers : JobDriver
    {
        ...
        {
            this.FailOn(() => !this.pawn.Map.lordManager.lords.Contains(this.pawn.CurJob.lord));
            ...
            if (lord != null)
            {
                for (int i = 0; i < lord.ownedPawns.Count; i++)
                {
                    ...
                    if (pawn2 != this.pawn)
                    {
                        ...
                        {
                            if (IsUsableCarrier(pawn2, this.pawn, false))
                            {
